...
## Обновление под ТЗ v1.0

Добавлено:

- тип `char` и символьные литералы (`'a'`, `'\n'`, `'\t'`, `'\\'`, `'\''`, `'\0'`);
- встроенная функция `assert(bool)`;
- целые литералы `0x...` и `0b...`;
- float-литералы с экспонентой (`1e3`, `1.5e-2`) и специальные значения `inf`, `NaN`;
- семантическая проверка наличия `main -> int32/int64`;
- `len` для массивов фиксированного размера;
- исправлены CMake targets и путь `run` на `examples/tests/basic.astra`.
